...
class TaskSortMethods {
  ...
  static int dateCreation(Task task1, Task task2) => task1.id.toLowerCase().compareTo(task2.id.toLowerCase());

  static int alphabetically(Task task1, Task task2) => task1.name.toLowerCase().compareTo(task2.name.toLowerCase());

  static int category(Task task1, Task task2) {
    if (task1.categoryId != null && task2.categoryId != null) {
      // Both have a category
      return task1.categoryId.toLowerCase().compareTo(task2.categoryId.toLowerCase());
    }
  ...
  static int dueDate(Task task1, Task task2) {
    if (task1.day != null && task2.day != null) {
      ...
      if (task1.day.year == task2.day.year && task1.day.month == task2.day.month && task1.day.day == task2.day.day) {
        // Both share the same day, so check to make sure both have a time
        if (task1.time != null && task2.time != null) {
          // Both have time, so sort by it
      ...
      return task1.day.compareTo(task2.day);
    } else {
