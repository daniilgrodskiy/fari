...
class _TaskWidgetState extends State<TaskWidget> with SingleTickerProviderStateMixin {
  ...
  @override
  void initState() {
    _scaleAnimationController = new AnimationController(vsync: this, duration: Duration(milliseconds: 100));
    _animation = CurvedAnimation(
      curve: Curves.easeOut,
      parent:  Tween<double>(begin: 1.0, end: 0.80).animate(_scaleAnimationController)
    );
    super.initState();
  }
          ...
          child: GestureDetector(
            ...
            child: task.categoryId != null && task.categoryId != ""
              ? StreamBuilder<Category>(
                  stream: widget.database.categoryStream(categoryId: task.categoryId),
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                      Category category = snapshot.data;
                      return _buildTaskWidgetContent(context, hasCategory: true, category: category, database: widget.database);
                    }
                    return Container();
                  }
                ) 
              : _buildTaskWidgetContent(context, hasCategory: false, database: widget.database)
          ),
        );
      }
    );
              ...
              child: Column(
                ...
                children: <Widget>[
                  Wrap(
                    children: <Widget>[
                      // Date
                      if((task.time != null) || (task.day != null && widget.showDate != null && widget.showDate)) 
                        ...
                        Container(
                          ...
                          child: Row(
                            ...
                            children: <Widget>[
                              ...
                              Icon(FontAwesomeIcons.calendarAlt, color: Colors.white54, size: 14.0,),
                              SizedBox(width: 5.0,),
          ...
          children: <Widget>[
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                color: task.isCompleted ? Colors.green[200].withAlpha(150) : Colors.white,
            ...
            if (task.isCompleted)
              Container(
                width: 35.0,
                height: 35.0,
                child: Icon(FontAwesomeIcons.check, color: Colors.green[300], size: 17,),
              ),
          ],
        ),
      ),
  ...
  Widget _buildCategory(Category category) {
    return Stack(
      children: <Widget>[
          ...
          decoration: BoxDecoration(
            border: Border.all(color: HexColor(category.color).withAlpha(100), width: 1.0),
            borderRadius: BorderRadius.circular(5.0),
